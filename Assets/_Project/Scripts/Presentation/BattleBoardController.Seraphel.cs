...
public sealed partial class BattleBoardController
{
	private IEnumerator ExecutePlayerTurnAgainstSeraphel(BattleCardState attacker, BattleCardState boss)
	{
		...
		SetMessage(GameText.Format(GameTextKeys.Campaign.SeraphelDamaged, attacker.Card.Name, defense.Damage, defense.HitPointsAfter, activeSeraphelBoss.MaxHitPoints));
	...
	private IEnumerator TransformSeraphelToPhaseTwo(BattleCardState boss)
	{
		...
		SetMessage(GameText.Get(GameTextKeys.Campaign.SeraphelPhaseTwo));
	...
	private IEnumerator ExecuteSeraphelBossTurn(BattleCardState boss)
	{
		...
		SetMessage(GameText.Format(GameTextKeys.Campaign.SeraphelJudgement, defender.Card.Name));
		bool hidden = HideMessagePanelForDiceRoll();
		PlayRollingDiceSfx();
		...
		if (attack.AttackSucceeded)
		{
			...
			if (defender.SeraphelSeals >= SeraphelBoss.SealExecutionThreshold)
			{
				...
				PlayDeathCardSfx();
				SetMessage(GameText.Format(GameTextKeys.Campaign.SeraphelThreeSeals, defender.Card.Name));
				yield return PlayTimelineAwareDefeatAnimation(defender, boss.Card.HeroClass);
			}
			else
			{
				SetMessage(GameText.Format(GameTextKeys.Campaign.SeraphelSealsApplied, defender.Card.Name, attack.SealsApplied, defender.SeraphelSeals, defender.SeraphelSeals * SeraphelBoss.DamagePerSeal));
			}
		}
		else
		{
			SetMessage(GameText.Format(GameTextKeys.Campaign.SeraphelJudgementResisted, defender.Card.Name));
		}
